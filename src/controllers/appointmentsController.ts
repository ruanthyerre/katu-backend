...
/**
 * Validate that the provided professionalId and patientId (if any) exist.
 * Throws an object { status, error } to be handled by callers.
 */
async function validateForeignKeys(professionalId?: string | null, patientId?: string | null) {
  if (professionalId) {
    const prof = await prisma.professional.findUnique({ where: { id: professionalId } });
    if (!prof) throw { status: 400, error: "invalid_professional" };
  }
  if (patientId) {
    const pat = await prisma.patient.findUnique({ where: { id: patientId } });
    if (!pat) throw { status: 400, error: "invalid_patient" };
  }
}
...
export async function updateAppointment(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const updates: any = {};
    const allowed = ["professionalId","patientId","clinicId","startAt","endAt","status"];
    ...
    // validate fk for new professional/patient (if provided)
    await validateForeignKeys(professionalId, patientId);
